...
function setPage(p) {
  ...
  if (isEmptyQuery.value) {
    router.push({ path: "/browse", query: { page: np } }).catch(() => {});
    return;
  }
  router.push({ path: "/browse", query: { q: queryTrimmed.value, page: np } }).catch(() => {});
}
  ...
  <section class="page">
    <header class="topbar">
      <div class="topbar-inner">
        <RouterLink class="brand" to="/" aria-label="Registry home">
        ...
        <nav class="actions" aria-label="Registry actions">
          <RouterLink class="action-btn" to="/publish">Publish</RouterLink>
        </nav>
      </div>
    </header>
